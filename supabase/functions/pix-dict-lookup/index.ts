...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!company_id || !pix_key) return new Response(JSON.stringify({ error: 'company_id and pix_key are required' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    ...
    if (config.provider === 'onz') {
      ...
      return new Response(JSON.stringify({
        success: true, name: '', cpf_cnpj: '', key_type: keyType, key: pix_key,
        bank_name: '', agency: '', account: '', account_type: '', end2end_id: '', ispb: '',
        provider: 'onz', note: 'ONZ valida a chave no momento do pagamento.',
      ...
      if (!tokenResponse.ok) return new Response(JSON.stringify({ error: 'Falha ao autenticar com provedor' }), { status: 502, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      ...
      if (!dictResponse.ok) {
        ...
        return new Response(JSON.stringify({ success: false, error: errorMessage, details: dictData }), { status: dictResponse.status === 404 ? 404 : 502, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
