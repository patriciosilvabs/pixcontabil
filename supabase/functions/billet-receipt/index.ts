...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (config.provider === 'onz') {
      ...
      const receiptUrl = `${config.base_url}/api/v2/billets/receipts/${billetExternalId}`;
      const result = await callPixMobileProxy(receiptUrl, 'GET', {
        'Authorization': `Bearer ${access_token}`,
        'Content-Type': 'application/json',
      });

      if (result.status >= 400 || !result.data) return new Response(JSON.stringify({ error: 'Comprovante ainda não disponível', status: result.data?.status }), { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });

      const pdfBase64 = result.data.pdf || result.data.receipt || result.data.pdf_base64;
      if (!pdfBase64) return new Response(JSON.stringify({ error: 'Comprovante ainda não disponível' }), { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      ...
      try {
        ...
        if (!billetResponse.ok) return new Response(JSON.stringify({ error: 'Failed to get billet data', provider_error: JSON.stringify(billetData) }), { status: 502, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });

        const receiptUrl = billetData.receipt_url || billetData.bank_receipt_url;
        if (!receiptUrl) return new Response(JSON.stringify({ error: 'Comprovante ainda não disponível', status: billetData.status }), { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
