...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!config) return new Response(JSON.stringify({ error: 'Configuração Pix não encontrada para pagamento de boletos.' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    ...
    if (config.provider === 'onz') {
      ...
      try {
        ...
        if (!batchResponse.ok) return new Response(JSON.stringify({ error: 'Falha ao pagar boleto via Transfeera', provider_error: JSON.stringify(paymentData) }), { status: 502, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      } catch (e) {
      ...
      if (insertError) return new Response(JSON.stringify({ error: 'Failed to save transaction' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
